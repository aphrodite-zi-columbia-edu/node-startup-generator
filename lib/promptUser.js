...
function searchCurDir() {
  var f = finder(process.cwd());
  var next = f.next();
  if (next.value)
    return path.dirname(next.value.__path)
  else
    return '';
}
console.log(searchCurDir());
...
function promptUser(callback) {
  ...
  },
  {
    ...
    .then(function (answers) {
      ...
      return callback(null, templateLocals);
    })
...
if (require.main === module) {
  promptUser((error, locals) => {
    if (error) throw error;

    var result = require('../lib/template')(locals);
